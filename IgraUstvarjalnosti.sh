...
dirname="Igra Ustvarjalnosti"
abspath="$(cd "$(dirname "$dirname")" && pwd -P)/$(basename "$dirname")/"
tempdir=$(mktemp -d)
url="https://www.dlib.si/results/?=&query=%27rele%253dIgra%2bustvarjalnosti%27&pageSize=100&fformattypeserial=journal&sortDir=ASC&sort=date"
